...
export default function EvidenceIntelligencePage() {
  ...
  const fetchEvidence = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/evidence-intelligence?${params}`);
  ...
  const aiRecommend = async (group: EvidenceGroup) => {
    ...
    try {
      const res = await fetch("/api/evidence-intelligence", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          return (
            <GlassCard key={group.obligation_id} className="p-0 overflow-hidden">
                  ...
                  <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                    <div className="border-t border-white/[0.06] p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => aiRecommend(group)} disabled={aiLoading === group.obligation_id}
