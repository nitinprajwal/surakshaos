...
export async function GET() {
  try {
    ...
    if (!obligations || obligations.length === 0) {
      ...
      if (storedScores && storedScores.length > 0) return NextResponse.json(storedScores);
    ...
    if (scores.length > 0) {
      try {
        await supabase.from("readiness_scores").upsert(scores, { onConflict: "department", ignoreDuplicates: false });
      } catch {
    ...
    const { data: storedFinal } = await supabase.from("readiness_scores").select("*").order("score", { ascending: false });
    if (storedFinal && storedFinal.length > 0) return NextResponse.json(storedFinal);
