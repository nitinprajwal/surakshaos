...
export default function ImpactSimulationPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/impact").then(r => r.json()).then(data => setPastSims(data ?? []));
  }, []);
  ...
  const runSimulation = useCallback(async () => {
    ...
    try {
      ...
      fetch("/api/impact").then(r => r.json()).then(d => setPastSims(d ?? []));
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-6">
        <h3 className="text-sm font-semibold text-[#d4e4fa] mb-4">Run Impact Simulation</h3>
