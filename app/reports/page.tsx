...
interface ReportData {
  ...
  mapStats: { backlog: number; in_progress: number; review: number; completed: number };
  complianceScore: number; generatedAt: string;
}

async function fetchReportData(): Promise<ReportData> {
  const [oblRes, docRes, mapRes] = await Promise.all([
    supabase.from("obligations").select("id,status,priority,department"),
    supabase.from("documents").select("id,status"),
    supabase.from("map_cards").select("id,status"),
  ]);
  ...
  const atRisk = obls.filter(o => o.status === "at_risk").length;
  const overdue = obls.filter(o => o.status === "overdue").length;
  const inProgress = obls.filter(o => o.status === "in_progress").length;
  const pendingReview = obls.filter(o => o.status === "pending_review").length;
  ...
  const byDepartment = Array.from(deptMap.entries()).map(([dept, v]) => ({ dept, count: v.count, compliant: v.compliant })).sort((a,b) => b.count - a.count);
  ...
  return {
    totalObligations: total, compliant, atRisk, overdue, inProgress, pendingReview,
    totalDocs: docs.length, processedDocs: docs.filter(d => d.status === "processed").length,
    byDepartment, byPriority,
    mapStats: {
      backlog: maps.filter(m => m.status === "backlog").length,
      in_progress: maps.filter(m => m.status === "in_progress").length,
      review: maps.filter(m => m.status === "review").length,
      completed: maps.filter(m => m.status === "completed").length,
    },
...
export default function ReportsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const d = await fetchReportData(); setData(d); }
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ...
      [],
      ["MAP BOARD STATS"],
      ["Backlog", data.mapStats.backlog],
      ["In Progress", data.mapStats.in_progress],
      ["Under Review", data.mapStats.review],
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <GlassCard className="p-5">
              <h3 className="text-sm font-semibold text-[#d4e4fa] mb-4 flex items-center gap-2"><Clock className="w-4 h-4 text-[#b0c6ff]" />MAP Board Status</h3>
              <div className="space-y-3">
                {[
                  { label: "Backlog", count: data.mapStats.backlog, color: "bg-slate-400" },
                  { label: "In Progress", count: data.mapStats.in_progress, color: "bg-[#b0c6ff]" },
                  { label: "Under Review", count: data.mapStats.review, color: "bg-amber-400" },
