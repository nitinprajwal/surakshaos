...
function EvidencePanel({ obligationId }: { obligationId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/evidence?obligation_id=${encodeURIComponent(obligationId)}`)
  ...
  const addEvidence = async () => {
    ...
    setAdding(true);
    try {
      const res = await fetch("/api/evidence", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ obligation_id: obligationId, title: newTitle }) });
  ...
  const toggleCollected = async (ev: EvidenceItem) => {
    ...
    try {
      const res = await fetch(`/api/evidence?id=${ev.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ collected }) });
      if (!res.ok) throw new Error("Failed");
...
export default function ObligationsPage() {
  ...
  const filteredObligations = obligations.filter(obl => {
    ...
    const matchSearch = !q || obl.title.toLowerCase().includes(q) || obl.source.toLowerCase().includes(q) || obl.department.toLowerCase().includes(q);
  ...
  const exportCsv = useCallback(() => {
    ...
    const headers = ["ID","Title","Source","Regulator","Department","Status","Priority","Confidence","Due Date","Citations"];
    const rows = filteredObligations.map((o: Obligation) => [o.id, `"${o.title.replace(/"/g,'""')}"`, `"${o.source.replace(/"/g,'""')}"`, o.regulator, o.department, o.status, o.priority, o.confidence, o.dueDate, `"${o.citations.join("; ").replace(/"/g,'""')}"`]);
  ...
  const handleDelete = async (obl: Obligation) => {
    ...
    try {
      const res = await fetch(`/api/obligations/${obl.id}`, { method: "DELETE" });
                        ...
                        <div className="px-6 py-5 bg-[#0d1c2d]/40 border-t border-white/[0.04]">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-[#8c90a1] leading-relaxed">{obl.title}</p>
